...
aim2_model_v4 <- function(median_data,training_cellines,proteomics,recompute = FALSE){
	library(ranger)
	library(progress)
	...
	if(recompute){
		...
		training_data <-  median_data %>% filter(data_purpose =="train")
		test_data <-  median_data %>% filter(data_purpose =="test")
		test_data$value = NA   # make sure we dont use it!
		...
		rg <- rg_data %>%
			...
			mutate(model = map(data,function(df){
				...
				rg.res <- ranger(formula = f , data = df.training,
								 mtry = 1000, # make sure to use most of the 
								 max.depth = 6,
								 verbose = T ,
		...
		if(FALSE) write_rds(rg,"./prediction_reports/aim2_model_v4.rds")
	}else{
		rg = read_rds("./prediction_reports/aim2_model_v4.rds")
	}
	...
	model_v4_predictions <- rg_predictions %>% unnest(predictions) %>% select(-value) %>%
