# aim2_model_v2: reduce the proteins by neighbourhood network

aim2_model_v2 <- function(median_interpolated_data,training_cellines,proteomics){
	library(ranger)
	library(progress)
	library(OmnipathR)
	...
	proteomics_neighbourhood_indicator = tibble(UID = names(proteomics)) %>% rowwise() %>%
		 mutate(included = any(strsplit(UID,".",fixed = T)[[1]] %in% neigbourhood))
	
	# we keep only a subset of the proteins: 618
	reduced_proteomics = bind_cols(cell_line=proteomics$cell_line,
								   proteomics[,proteomics_neighbourhood_indicator$included])
