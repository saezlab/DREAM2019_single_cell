---
title: "Leaderboards for Subchallenge III"
author: "Attila Gabor"
date: "11/25/2019"
...
# Overview
Here we report some statistics about subchallenge III and evaluate the leaderboard. 

```{r message=FALSE, warning=FALSE, paged.print=FALSE, results=FALSE, echo=FALSE}

LB <- read_csv("../submission_data/final_round/outdated/leaderboard_all_rounds_sc3.csv") %>%
...
LB %>% filter(status=="SCORED") %>%
	...
	ggtitle("Sub-challenge III: predict unseen treatment") +
	ylab("# of submissions") + 
	geom_vline(xintercept = 336.76,color = colors[[3]],size=2) + facet_grid( round~ .)
...
- The top 5 places are stable. 
- `mostyrice` could jump to place 6 if we consider best submission. 
- 
...
LB %>% filter(status == "SCORED") %>% 
	...
	ungroup() %>%
	mutate(submitterId = factor(submitterId,levels = filter(.,latest) %>% arrange(score) %>% filter(!duplicated(submitterId)) %>% pull(submitterId)))%>%
...
Team `GaoGao199696`  has two submissions at the same time with different scores. 
Submission time is accurate by minutes.
...
LB %>% filter(status == "SCORED") %>% 
	group_by(submitterId) %>%
	top_n(-1, score) %>%
	filter(!duplicated(submitterId)) %>%  # for duplicates with identical scores
	ungroup() %>%
	arrange(score) %>%
...
LB %>% filter(status == "SCORED") %>% 
	...
	ungroup() %>%
	arrange(score) %>%
	mutate(submitterId = factor(submitterId,levels = arrange(.,score) %>%filter(!duplicated(submitterId)) %>% pull(submitterId)))%>%
	ggplot(aes(submitterId,score)) + 
	geom_point(aes(color=round, shape=round),size = 2) + 
	theme_bw() + 
	theme(axis.text.x = element_text(angle = 90,hjust = 1)) +
