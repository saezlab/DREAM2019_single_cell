...
# SC3: compare the teams and combination of predictions, data-usage etc
# A. Gabor
...
scores_ordered_teams <- read_rds("./prediction_combinations/SC3/SC3_ordered_combination_stats.rds")
scores_random_teams <- read_rds("./prediction_combinations/SC3/SC3_random_subs_scores_stats.rds")


RF_combination <- read_rds("./submission_data/combination_scoring/SC3/L4O_CV_scores.rds") %>%
    summarise_at(3:9,~sqrt(sum(.^2)))


method_usage <- readxl::read_xlsx("./methods/sc3_methods_processed.xlsx",sheet = 2)
...
# Basic statistics:
teams_score = conditional_error_stats %>% 
    gather(teams,cond_score,-cond_id) %>% 
    separate(cond_id,into = c("cell_line","treatment","time"),sep = "_")%>%
    group_by(teams) %>% 
    summarise(score = sqrt(sum(cond_score)))  %>% arrange(score)

teams_score %>% slice(1:7) %>% summarise(min_score = min(score),
                                         max_score = max(score),
                                         mean_score = mean(score),
                                         median_score = median(score),
                                         std_score = sd(score))
teams_score %>% slice(8:14) %>% summarise(min_score = min(score),
                                         max_score = max(score),
                                         mean_score = mean(score),
                                         median_score = median(score),
                                         std_score = sd(score))
teams_score %>% 
    ungroup()  %>% summarise(min_score = min(score),
                             ...
                             std_score = sd(score))
# challange winner
bootstrap_RMSE$icx_bxai %>% summary()
sd(bootstrap_RMSE$icx_bxai)
# second pos
bootstrap_RMSE$orangeballs %>% summary()
sd(bootstrap_RMSE$orangeballs)
...
# bootstrap_RMSE %>% 
    ...
    geom_hline(data=NULL,aes(yintercept = 51.7, color = "EGF prediction"))+
...
method_usage_plot <- method_usage %>% 
    select(-2,-4,-9, -10,-11,-13) %>% # remove data usage entities
...
data_usage_plot <- method_usage %>% 
    select(1,2,4,9, 10,11,13)%>%  # select ID and  data usage entities
    mutate(submitterId = factor(submitterId,levels = levels(ranked_teams))) %>%
...
plot_grid(rank_plot,
          method_usage_plot, 
          data_usage_plot,
          axis = "lr", align = "v", rel_heights =  c(1.5, .8, .9), ncol = 1)
ggsave("./publication/figures/figure4/sc3_leaderboard_anonym.pdf", width = 6,height = 5)
