...
teams = names(combined_statistics)[7:22]
...
plot_rmse_treatment <- error_terms %>% group_by(cell_line,treatment, time) %>%
    summarise(rmse = sqrt(sum(error^2)/n())) %>%
    ggplot() + geom_violin(aes(treatment,rmse),draw_quantiles = .5,fill= fill_color)  +
    theme_bw() + 
    guides(fill = "none") + 
    labs( ylab= NULL) + xlab("Treatment")
...
cowplot::plot_grid(plot_rmse_cell_line,plot_rmse_time,NULL, plot_rmse_treatment)
