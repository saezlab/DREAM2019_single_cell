# Analysing heterogeneity in combinations of predictions in SC2
...
combi <- readRDS("./prediction_combinations/SC2/L2O_CV_all_predictions.rds")
...
combi_sub <- combi %>%
  ...
  mutate(sample = map(prediction, ~sample_frac(., 0.1))) %>%
  ungroup() %>%
  select(-prediction) %>%
  unnest(sample)
...
conditions <- combi %>%
  ...
  distinct()
# Clustering and analysis per cell line-treatment condition for selected model
# For this step combi_sub is sampled 0.1 per model and condition
for (j in 1:length(unique(combi$model))) {
  print(unique(combi$model)[j])
  model <- unique(combi$model)[j]
  model_sub <- combi_sub %>%
    filter(model ==  unique(combi$model)[j])
...
combi_sub <- combi %>%
  group_by(model, cell_line, treatment, time) %>%
  mutate(sample = map(prediction, ~sample_frac(., 0.01))) %>%
  ungroup() %>%
  select(-prediction) %>%
  unnest(sample)

# Do UMAP dimensionslity reduction and apply hierarchical clustering per model
# Clustering over all cell lines, treatments and timepoints
# For this step combi_sub is samples with 0.01 per model and condition
if (TRUE) {
