...
for (i in 1:dim(conditions)[1]){
  ...
  pdf(paste0(TR, "_", CL, "/GS_UMAP_HDB_clust_HM.pdf"), height = 8, width = 4)
...
if (TRUE) {
  ...
  pdf("GS_UMAP_HDB_clust.pdf", height = 5, width = 5)
  print(GS_clusters %>%
          ggplot(aes(x=V1, y=V2, colour=umap_HDB_clust)) +
          geom_point(size=0.1) +
          labs(x = "UMAP V1", y = "UMAP V2", colour = "cluster",
  ...
  pdf("GS_UMAP_HDB_clust_HM.pdf", height = 8, width = 4)
  print(Heatmap(as.matrix(select(hm, -marker)), right_annotation = rowAnnotation(marker = anno_text(hm$marker)), 
                col = RColorBrewer::brewer.pal(9, "YlOrRd"), column_title = "HDB-UMAP clustering", 
                bottom_annotation = HeatmapAnnotation(cluster_size = anno_barplot(count(GS_clusters, umap_hdb_clust)$n)))
  )
  dev.off()
}
