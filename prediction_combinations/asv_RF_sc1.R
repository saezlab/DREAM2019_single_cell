# Combine predicitons of SC1; asv_RF
# Cross validation based on cell line
# Model per marker
# Random forest that takes the single cell predictions of the teams, treatment and time as variables
# RF predicts the single cell expression values
# RF always splits on time and treatment
...
asv_RF_predictions <- tibble()

# Save error per condition of predictions
asv_RF_pred_errors <- tibble()
for (i in 1:length(cell_lines)) {
  ...
  RF_formula <- paste0("standard ~", paste(c(submissions, "treatment", "time"), collapse = "+")) 
  ...
  RF_pred <- val_data_all %>%
    ...
    mutate(asv_RF_pred =  map2(data, prediction, function(x, y) {add_column(x, "asv_RF_pred" = y$predictions)})) %>%
    select(marker, asv_RF_pred) %>%
    unnest(cols = c(asv_RF_pred)) %>%
    ungroup() %>%
    select(-c(submissions, np_markers, n))
  RF_error <- RF_pred %>%
    group_by(cell_line, treatment, time, marker) %>%
    summarise(asv_RF_error = sqrt(sum((standard - asv_RF_pred)^2) / n())) %>%
    select(cell_line, treatment, time, marker, asv_RF_error) %>%
    ungroup()
  RF_val_score <- RF_error %>%
    pull(asv_RF_error) %>%
    mean()
  
  asv_RF_predictions <- bind_rows(asv_RF_predictions, RF_pred)
  asv_RF_pred_errors <- bind_rows(asv_RF_pred_errors, RF_error)
...
if(FALSE){
  saveRDS(scores, "./prediction_combinations/SC1/LOO_CV_asvRF_scores.rds")
  saveRDS(asv_RF_predictions, "./prediction_combinations/SC1/LOO_CV_asvRF_all_predictions.rds")
  saveRDS(asv_RF_pred_errors, "./prediction_combinations/SC1/LOO_CV_asvRF_pred_errors.rds")
}
