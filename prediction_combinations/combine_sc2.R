# To combine predictions of round 2
library(tidyverse)
...
subchallenge <-  "prediction_combinations/SC2"
submission_folder <- "./submission_data/round3/SC2"
...
scores <- predictions %>% 
  ...
  nest() %>% 
  ungroup() %>% 
  mutate(score = map(data, ~score_sc2(., validation_data))) %>%
  select(prediction, score) %>%
  unnest()
saveRDS(scores, "prediction_combinations/SC2/SC2_prediction_scores.rds")
} else{scores <- readRDS("prediction_combinations/SC2/SC2_prediction_scores.rds")}
...
saveRDS(better_and_best, "prediction_combinations/SC2/SC2_performance.rds")
  ...
  repeated_scores <- readRDS("prediction_combinations/SC2/SC2_scores.rds")
