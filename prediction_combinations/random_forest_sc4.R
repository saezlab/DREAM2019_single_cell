...
np_markers <- sub_data_values %>% 
  ...
  colnames()
np_markers <- np_markers[np_markers!= "full0_p.PLCg2"] #PLCg2 someimes na
...
for (i in 1:length(cell_lines)) {
  ...
  MDT <- randomForest(as.formula(paste0("best_sub ~", features)), data=train_data_err, ntree = 500, 
                      localImp = TRUE)
  #Arbiter
...
if (TRUE) {
  saveRDS(scores, "./prediction_combinations/SC4/LOO_CV_RF_scores.rds")
  saveRDS(feature_importance, "./prediction_combinations/SC4/LOO_CV_RF_feature_importance.rds")
  saveRDS(all_predictions, "./prediction_combinations/SC4/LOO_CV_RF_all_predictions.rds")
  
}

CV_scores <- readRDS("./prediction_combinations/SC4/LOO_CV_RF_scores.rds")
CV_features <- readRDS("./prediction_combinations/SC4/LOO_CV_RF_feature_importance.rds")
...
CV_features  %>%
  ...
  ggplot(aes(reorder(feature, -importance), importance/(5*21), fill=feature)) +
  geom_col() +
