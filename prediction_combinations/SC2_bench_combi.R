# To compare combined prediction scores to bechmark, random, replicates for SC2 
library(tidyverse)
...
validation_file <- "~/Seafile/validation_data/sc2gold.csv"
...
valB <- validation_data %>% left_join(select(fileID, fileID, time_course)) %>%
  ...
  select(-time_course)
score_replicates <- score_sc2(valA, valB)

#better_and_best <- readRDS("prediction_combinations/SC2/SC2_performance.rds")

single_model_scores <- readRDS("prediction_combinations/SC2/SC2_prediction_scores.rds") %>% select(prediction, score) %>% rename(model = prediction)
combi_model_scores <- readRDS("prediction_combinations/SC2/SC2_scores.rds") %>% gather(model, score)
random_scores <- readRDS("prediction_combinations/SC2/SC2_random_scores.rds")
attila_score <- read_csv( "~/Seafile/dry_run/aim1_2_1_predictions.csv") %>% score_sc2(validation_data)


single_scores <- tibble(model = c("Attila", "best_single", "replicatesT0"),
                       score = c(attila_score, min(single_model_scores$score), score_replicates))
