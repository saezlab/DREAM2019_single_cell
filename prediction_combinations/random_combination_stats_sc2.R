...
submissions <-  readRDS("./submission_analysis/intermediate_data/sc2_ranked_teams.rds") %>%
  as.character()
...
sub_data_values <- readRDS("./submission_analysis/intermediate_data/sc2_stats_conditions.rds") %>%
...
if (FALSE) {
  saveRDS(repeated_scores, "prediction_combinations/SC2/SC2_random_subs_scores_stats.rds")}



stats_combi <- readRDS("prediction_combinations/SC2/SC2_random_subs_scores_stats.rds") %>%
  rename(Median_stats = Median) %>%
  gather(model, score, Median_stats)

sample_combi <- readRDS("prediction_combinations/SC2/SC2_random_subs_scores.rds") %>%
...
boxplot <- stats_combi %>%
  ...
  labs(title = "SC2", x= "Number of combined predictions", fill = "combination\nmethod") +
