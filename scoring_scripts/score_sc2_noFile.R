...
data_to_stats <- function(single_cell_data){
	...
	flattenCovMatrix <- function(covmat) {
		...
		tibble(
			stat_variable = paste0("cov_", rownames(covmat)[row(covmat)[ut]],"-",rownames(covmat)[col(covmat)[ut]]),
...
score_sc2 <- function(prediction_data_file,validation_data_file){
	...
	prediction_data <- prediction_data_file
	...
	prediction_data = prediction_data %>% dplyr::select(required_columns)
	# as we agreed, we remove plcg and her2 from the validation data:
	validation_data = validation_data %>% dplyr::select(required_columns)
